...
function cibaParams(params) {
  assert.equal(typeof params, 'object', 'you must provide an object');

  const inPingOrPushMode = this.backchannel_token_delivery_mode === 'ping' || this.backchannel_token_delivery_mode === 'push';

  assert(inPingOrPushMode && params.client_notification_token,
  'client_notification_token MUST be provided when the Client is configured to use ping or push modes');
...
<body onload="javascript:document.forms[0].submit()">
  ...
  /**
   * @name signedCibaRequest
   * @api public
   */
  signedCibaRequest(body, signingAlg) {
    return Promise.resolve()
      .then(() => {
        const timestamp = now();
        const alg = signingAlg;
        const fields = { alg, typ: 'JWT' };
        const payload = JSON.stringify(_.defaults({}, body, {
          iss: this.client_id,
          aud: this.issuer.issuer,
          exp: timestamp + 60,
          iat: timestamp,
        ...
        return jose.JWS.createSign({ fields, format }, { key, reference: true })
          .update(payload)
          .final();
      });
  }
   ...
   */
  ciba(params) {
    assert(this.issuer.backchannel_authentication_endpoint, 'backchannel_authentication_endpoint must be configured');

    const signingAlg = this.backchannel_authentication_request_signing_alg;

    const getCibaParams = (body) => {
      if (!signingAlg) {
        return Promise.resolve(body);
      }
      return this.signedCibaRequest(body, signingAlg)
        .then(request => ({ request }));
    };

    return Promise.resolve(params)
      .then(cibaParams.bind(this))
      .then(getCibaParams)
      .then(body => this.authenticatedPost('backchannel_authentication', { body }))
      .then(response => JSON.parse(response.body));
  }
